classdef model_exp_environement_multi_PPF_phase < rl.env.MATLABEnvironment
...
%     end 
    ...
    methods             
        ...
        function this = model_exp_environement_multi_PPF_phase(ObservationInfo, ActionInfo, dt, time_input, Disturbance, Sys, reward_type,rwd_coef)
        ...
        function [Observation,Reward,IsDone,LoggedSignals] = step(this,Action)
            ...
            this.K_value = round(-10.^((Action(1:3)./10./2+1/2)*10),-3);%=-10.^(10*[Action(1:3)])+1;
            this.Xi_value =round([Action(4:6)./10./2+1/2],2);%*1000*2*pi;
            this.Ome_value =round([Action(7:9)./10./2+1/2]*1000*2*pi);
...
%             this.Ome_value =[Action(7:9)./2+1/2]*1000*2*pi;
            ...
            this.H=[Crtl(1) 0       0;
                                                0      Crtl(2)  0;
                                                0      0       Crtl(3)];
%             this.H=1000*2*pi/(s+1000*2*pi)*[Crtl(1) 0       0;
%                                                 0      Crtl(2)  0;
%                                                 0      0       Crtl(3)];
...
%             end
            ...
            if  ~IsStable || ((Gm_dB(1)<Gmref && Wcg1>10) || (Gm_dB(2)<Gmref && Wcg2>10) || (Gm_dB(3)<Gmref && Wcg3>10) || Pm1<Pmref && Wcp1>10 || (Pm2<Pmref && Wcp2>10) || (Pm3<Pmref && Wcp3>10) || (this.rms_y2_ddot>10) || (isnan(this.rms_y2_ddot)) || (this.rms_y2_ddot == Inf))  
               ...
               corr1=Gm_dB(Gm_dB<Gmref) 
               corr1=corr1(corr1>0)
               corr2=Gm_dB(Gm_dB<0)
               corr2=corr2(corr2>-10)
               corr3=this.Pm(this.Pm<180) 
               ...
               %corr=corr(corr>0)
                 %Reward = -abs(20*log10(Reward))+(Gm_dB(1)/10+Gm_dB(2)/10+Gm_dB(3)/10)%-20*log10(3);
                  ...
                  if IsStable 
                    disp('IsStable')
                    Reward = 20*log10(Reward)*(abs(prod(corr1/10)))*(abs(prod(corr3/180)))
                  elseif isempty(corr2)
                     Reward = -1
                  else
                    Reward = -(abs(prod(corr2/10)))*(abs(prod(corr3/180)))%-20*log10(3);
                  end
% 
% 
%               if (Reward > 3)
